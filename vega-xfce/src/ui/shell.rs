...
use super::widgets::{self, ActionRow, PreferencesGroup};
use super::{
...
impl VegaShell {
    pub fn new() -> Self {
        ...
        brand.append(&mark);
        brand.append(&gtk::Label::new(Some("Lyra Vega XFCE")));
        ...
        let title =
            widgets::window_title("Lyra Vega XFCE", &gettext("Centro de Controle Empresarial"));
...
fn about_page(
    ...
    let content = page_box(
        &gettext("Sobre"),
        "Lyra Vega XFCE - Enterprise Control Center",
    );
    content.append(&card(&gettext("Versões"), versions).widget);
    let system = PreferencesGroup::builder()
        .title(gettext("Sistema"))
        .build();
    system.add(&property_row(&gettext("Distribuição"), distro));
    let logo_row = ActionRow::builder().title(gettext("Logo")).build();
    logo_row.add_suffix(logo);
    system.add(&logo_row);
    system.add(&property_row(&gettext("Camada da comunidade"), channel));
    content.append(&system.widget);
    let product = PreferencesGroup::builder()
...
fn card(title: &str, value: &gtk::Label) -> PreferencesGroup {
...
fn property_row(title: &str, value: &gtk::Label) -> ActionRow {
