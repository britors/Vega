...
impl ServicesPage {
    ...
    pub fn show(&self, items: Vec<ManagedService>) {
        ...
        for service in &items {
            ...
            connect_row_action(&enable_button, &self.list, &row.widget, &self.enable);
            connect_row_action(&running_button, &self.list, &row.widget, &self.running);
            connect_row_action(&restart_button, &self.list, &row.widget, &self.restart);
