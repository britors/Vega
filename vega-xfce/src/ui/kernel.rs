...
impl KernelPage {
    pub fn new() -> Self {
        ...
        let boot = PreferencesGroup::builder()
...
fn fill_list(list: &gtk::ListBox, values: &[String], empty: &str) {
    ...
    if values.is_empty() {
        ...
        list.append(&ActionRow::builder().title(empty).build().widget);
        return;
    }
    ...
    for value in values {
        list.append(
            &ActionRow::builder()
                .title(gtk::glib::markup_escape_text(value).to_string())
                .build()
                .widget,
        );
    }
...
fn property(title: &str, value: &gtk::Label) -> ActionRow {
