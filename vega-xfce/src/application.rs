...
pub const APPLICATION_ID: &str = "org.lyraos.VegaXfce";
...
fn install_style() {
    gtk::Window::set_default_icon_name("vega-xfce");
...
// GTK4 sem libadwaita não tem `adw::StyleManager`/cores nomeadas do tema —
// aproximação própria da paleta clara/escura da Adwaita, escolhida uma vez
// na inicialização a partir da preferência do sistema (sem trocar ao vivo).
const DARK_PALETTE: &str = "\
@define-color lyra_night #1e1e1e;
@define-color lyra_night_alt #2a2a2a;
@define-color lyra_surface #303030;
@define-color lyra_surface_raised #383838;
@define-color lyra_border alpha(#ffffff, 0.14);
@define-color lyra_text #ffffff;
@define-color lyra_muted alpha(#ffffff, 0.72);
";
const LIGHT_PALETTE: &str = "\
@define-color lyra_night #fafafa;
@define-color lyra_night_alt #ebebeb;
@define-color lyra_surface #ffffff;
@define-color lyra_surface_raised #f0f0f0;
@define-color lyra_border alpha(#1e1e1e, 0.14);
@define-color lyra_text #1e1e1e;
@define-color lyra_muted alpha(#1e1e1e, 0.72);
";
...
fn build_window(app: &gtk::Application) {
    ...
    let window = gtk::ApplicationWindow::builder()
        ...
        .child(&shell.root)
        .build();
    window.set_icon_name(Some("vega-xfce"));

    update_content(shell, identity.version.clone(), window.clone());
    window.present();
}

fn update_content(shell: VegaShell, ui_version: String, window: gtk::ApplicationWindow) {
    glib::MainContext::default().spawn_local(async move {
        ...
        configure_services(&shell, &window, dbus.clone());
        configure_users(&shell, &window, dbus.clone());
        configure_backup(&shell, &window, dbus.clone());
        configure_snapshots(&shell, &window, dbus.clone());
        configure_kernel(&shell, &window, dbus.clone());
        configure_datetime(&shell, &window, dbus.clone());
        configure_screen(&shell, &window, dbus.clone());
        configure_storage(&shell, &window, dbus.clone());
...
fn configure_users(shell: &VegaShell, window: &gtk::ApplicationWindow, dbus: VegaDbus) {
    ...
    page.create.connect_clicked(move |_| {
        ...
        let window = create_window.clone();
        glib::MainContext::default().spawn_local(async move {
            let confirmed = widgets::confirm(
                &window,
...
fn connect_user_action(
    ...
    button.connect_clicked(move |_| {
        ...
        glib::MainContext::default().spawn_local(async move {
            if !widgets::confirm(&window, &title, &message, &confirm_label).await {
                return;
            }
            page.set_busy(true);
            page.status
...
fn configure_services(shell: &VegaShell, window: &gtk::ApplicationWindow, dbus: VegaDbus) {
...
fn connect_service_action(
    ...
    button.connect_clicked(move |_| {
        ...
        let title = gettext("{verb} serviço?").replace("{verb}", &verb);
        let message = gettext("{label} ({name}) {detail}.")
        ...
        let window = window.clone();
        glib::MainContext::default().spawn_local(async move {
            if !widgets::confirm(&window, &title, &message, &verb).await {
                return;
            }
            page.set_busy(true);
...
fn configure_software(shell: &VegaShell, window: &gtk::ApplicationWindow, dbus: VegaDbus) {
    ...
    page.optimize_mirrors.connect_clicked(move |_| {
        ...
        let window = mirrors_window.clone();
        glib::MainContext::default().spawn_local(async move {
            let confirmed = widgets::confirm(
                &window,
                &gettext("Otimizar mirrors?"),
                &gettext(
    ...
    page.global_action.connect_clicked(move |_| {
        ...
        let window = global_window.clone();
        glib::MainContext::default().spawn_local(async move {
            if !widgets::confirm(&window, &heading, &body, &confirm_label).await {
                return;
            }
    ...
    action_page.action.clone().connect_clicked(move |_| {
        ...
        glib::MainContext::default().spawn_local(async move {
            ...
            let confirmed = if let Some(pkgbuild) = pkgbuild {
                confirm_aur_install(&window, &package.name, &pkgbuild).await
            } else {
                widgets::confirm(
                    &window,
                    &gettext("{verb} {name}?")
...
fn connect_repository_toggle(
    ...
    page.clone().connect_repository_toggle(move |repository| {
        ...
        let title = gettext("{verb} repositório?").replace("{verb}", &verb);
        let page = page.clone();
        let client = dbus.software();
        let window = window.clone();
        glib::MainContext::default().spawn_local(async move {
            if !widgets::confirm(&window, &title, &repository.name, &verb).await {
                return;
            }
...
async fn confirm_aur_install(window: &gtk::ApplicationWindow, name: &str, pkgbuild: &str) -> bool {
    ...
    widgets::confirm_with_extra(
        window,
        &gettext("Revisar PKGBUILD de {name}").replace("{name}", name),
        &gettext(
            "Pacotes da comunidade executam instruções de compilação. Revise o conteúdo antes de continuar.",
        ),
        &scroll,
        &gettext("Revisei e instalar"),
    )
    .await
}
...
fn configure_bluetooth(shell: &VegaShell, window: &gtk::ApplicationWindow, dbus: VegaDbus) {
    ...
    let power_window = window.clone();
    page.power.connect_clicked(move |_| {
        ...
        let window = power_window.clone();
        glib::MainContext::default().spawn_local(async move {
            let confirmed = widgets::confirm(
                &window,
                &gettext("{verb} Bluetooth?").replace("{verb}", &verb),
                &if enable {
    ...
    page.scan.connect_clicked(move |_| {
    ...
    let device_window = window.clone();
    page.device_action.connect_clicked(move |_| {
        ...
        let window = device_window.clone();
        glib::MainContext::default().spawn_local(async move {
            let confirmed = widgets::confirm(
                &window,
                &gettext("{action} dispositivo?").replace("{action}", &action),
                &format!("{} • {}", device.display_name(), device.address),
                &action,
            )
            .await;
            if !confirmed {
    ...
    page.send_file.connect_clicked(move |_| {
        let Some(device) = send_page.selected_device() else {
            return;
        };
        let dialog = gtk::FileDialog::builder()
            .title(gettext("Enviar arquivo por Bluetooth"))
            .build();
        let page = send_page.clone();
        let dbus = send_dbus.clone();
        let window = send_window.clone();
        glib::MainContext::default().spawn_local(async move {
            ...
            let confirmed = widgets::confirm(
                &window,
                &gettext("Enviar arquivo por Bluetooth?"),
            ...
            match dbus
                .bluetooth()
                .send_file(&device.address, &display_path)
                .await
            {
                Ok(()) => page
                    .status
    ...
    page.receive_files.connect_clicked(move |_| {
        let dialog = gtk::FileDialog::builder()
            .title(gettext("Pasta para arquivos recebidos"))
            .build();
        let page = receive_page.clone();
        let dbus = receive_dbus.clone();
        let window = receive_window.clone();
        glib::MainContext::default().spawn_local(async move {
            ...
            let confirmed = widgets::confirm(
                &window,
                &gettext("Ativar recebimento Bluetooth?"),
                &gettext(
...
fn configure_network(shell: &VegaShell, window: &gtk::ApplicationWindow, dbus: VegaDbus) {
    ...
    let interface_window = window.clone();
    page.interface_action.connect_clicked(move |_| {
        ...
        let window = interface_window.clone();
        glib::MainContext::default().spawn_local(async move {
            let confirmed = widgets::confirm_with_extra(
                &window,
                &gettext("Configurar IPv4 estático?"),
                &gettext(
    ...
    page.connect_wifi_action(move |network| {
        ...
        glib::MainContext::default().spawn_local(async move {
            ...
            let body = if disconnect {
                gettext("{ssid} será desconectada deste dispositivo.")
            } else {
                gettext("{ssid} será conectada pelo NetworkManager.")
            }
            .replace("{ssid}", &network.ssid);
            let confirm_label = if disconnect {
                gettext("Desconectar")
            } else {
                gettext("Conectar")
            };
            let confirmed = if needs_password {
                widgets::confirm_with_extra(&window, &title, &body, &password, &confirm_label).await
            } else {
                widgets::confirm(&window, &title, &body, &confirm_label).await
            };
            if !confirmed {
                return;
            }
    ...
    let proxy_window = window.clone();
    page.proxy_apply.connect_clicked(move |_| {
    ...
    page.vpn_import.connect_clicked(move |_| {
        ...
        let dialog = gtk::FileDialog::builder()
            .title(gettext("Importar perfil OpenVPN"))
            .filters(&filters)
            .build();
        let page = vpn_page.clone();
        let dbus = vpn_dbus.clone();
        let window = vpn_window.clone();
        glib::MainContext::default().spawn_local(async move {
            let Ok(file) = dialog.open_future(Some(&window)).await else {
                return;
            };
            let Some(path) = file.path() else {
                page.vpn_status
                    .set_label(&gettext("Selecione um arquivo local de perfil OpenVPN."));
                return;
            };
            if !path
                .extension()
            ...
            let confirmed = widgets::confirm(
                &window,
                &gettext("Importar perfil OpenVPN?"),
                &gettext(
    ...
    let firewall_button = page.firewall_action.clone();
    let firewall_window = window.clone();
    firewall_button.connect_clicked(move |_| {
        ...
        let window = firewall_window.clone();
        glib::MainContext::default().spawn_local(async move {
            let confirmed = widgets::confirm(
                &window,
                &gettext("{verb} serviço no firewall?").replace("{verb}", &verb),
...
fn configure_storage(shell: &VegaShell, window: &gtk::ApplicationWindow, dbus: VegaDbus) {
    ...
    shell.storage.action.connect_clicked(move |_| {
        ...
        glib::MainContext::default().spawn_local(async move {
            let confirmed = widgets::confirm(
                &window,
                &gettext("{verb} volume?").replace("{verb}", &verb),
                &format!(
                    "{} ({})",
                    volume.path,
...
fn configure_screen(shell: &VegaShell, window: &gtk::ApplicationWindow, dbus: VegaDbus) {
    configure_wallpaper_tab(&shell.screen.wallpaper);
    configure_screensaver_tab(&shell.screen.screensaver);
    configure_monitor_tab(&shell.monitor, window, dbus);
}
...
fn configure_monitor_tab(
    ...
    page.connect_kill(move |process| {
        ...
        let dbus = dbus.clone();
        let window = kill_window.clone();
        glib::MainContext::default().spawn_local(async move {
            let confirmed = widgets::confirm(
                &window,
                &gettext("Encerrar processo?"),
...
fn configure_datetime(shell: &VegaShell, window: &gtk::ApplicationWindow, dbus: VegaDbus) {
    ...
    shell.datetime.apply.connect_clicked(move |_| {
        ...
        glib::MainContext::default().spawn_local(async move {
            let confirmed = widgets::confirm(
                &window,
                &gettext("Alterar data, hora e idioma?"),
                &gettext(
...
fn configure_kernel(shell: &VegaShell, window: &gtk::ApplicationWindow, dbus: VegaDbus) {
    ...
    page.install.connect_clicked(move |_| {
        ...
        glib::MainContext::default().spawn_local(async move {
            ...
            match dbus.kernel().install(&kernel).await {
                Ok(transaction_id) => {
                    page.status.set_label(
                        &gettext("Instalação iniciada (transação #{id}). Atualizando a lista…")
    ...
    page.remove.connect_clicked(move |_| {
        ...
        let window = remove_window.clone();
        glib::MainContext::default().spawn_local(async move {
            let confirmed = widgets::confirm(
                &window,
                &gettext("Remover kernel?"),
                &gettext(
    ...
    shell.kernel.apply_boot.connect_clicked(move |_| {
        ...
        let window = boot_window.clone();
        glib::MainContext::default().spawn_local(async move {
            let confirmed = widgets::confirm(
                &window,
                &gettext("Alterar configuração de boot?"),
                &gettext(
...
fn configure_snapshots(shell: &VegaShell, window: &gtk::ApplicationWindow, dbus: VegaDbus) {
    ...
    let compare_page = page.clone();
    let compare_dbus = dbus.clone();
    page.connect_compare(move |snapshot, _button| {
        compare_page
            .comparison
    ...
    page.create.connect_clicked(move |_| {
        ...
        glib::MainContext::default().spawn_local(async move {
            let confirmed = widgets::confirm_with_extra(
                &window,
                &gettext("Criar ponto de restauração?"),
                &gettext("O snapshot será criado pelo backend disponível no sistema."),
                &description,
                &gettext("Criar"),
            )
            .await;
    ...
    page.connect_delete(move |snapshot, button| {
        let page = delete_page.clone();
        let client = delete_dbus.snapshots();
        let window = delete_window.clone();
        glib::MainContext::default().spawn_local(async move {
            let confirmed = widgets::confirm(
                &window,
                &gettext("Excluir ponto de restauração?"),
    ...
    let rollback_page = page.clone();
    let rollback_dbus = dbus.clone();
    let rollback_window = window.clone();
    page.connect_apply(move |snapshot, button| {
        button.set_sensitive(false);
        rollback_page
            .comparison
        ...
        let window = rollback_window.clone();
        glib::MainContext::default().spawn_local(async move {
    ...
    let retention_page = page.clone();
    let retention_dbus = dbus;
    let retention_window = window.clone();
    page.apply_retention.connect_clicked(move |_| {
        ...
        let window = retention_window.clone();
        glib::MainContext::default().spawn_local(async move {
            let confirmed = widgets::confirm(
                &window,
                &gettext("Alterar política de retenção?"),
                &gettext(
...
fn configure_backup(shell: &VegaShell, window: &gtk::ApplicationWindow, dbus: VegaDbus) {
    ...
    page.new_config.connect_clicked(move |_| {
        ...
        glib::MainContext::default().spawn_local(async move {
            let confirmed = widgets::confirm_with_extra(
                &window,
                &gettext("Nova configuração de backup"),
                &gettext("Os caminhos serão validados pelo vegad antes de serem salvos."),
                &form,
                &gettext("Criar"),
            )
            .await;
    ...
    page.delete_config.connect_clicked(move |_| {
        ...
        glib::MainContext::default().spawn_local(async move {
            let confirmed = widgets::confirm(
                &window,
                &gettext("Excluir configuração de backup?"),
                &gettext(
    ...
    page.restore_selected.connect_clicked(move |_| {
        ...
        if paths.is_empty() {
            restore_page
                .snapshot_paths
                .set_label(&gettext("Selecione ao menos um caminho para restaurar."));
            return;
        }
        ...
        let body = if replacing {
            ...
            gettext("{count} item(ns) serão restaurados em uma pasta separada dentro de {target}.")
        }
    ...
    shell.backup.run_now.connect_clicked(move |_| {
        ...
        let window = run_window.clone();
        glib::MainContext::default().spawn_local(async move {
            let confirmed = widgets::confirm(
                &window,
                &gettext("Executar backup agora?"),
...
fn configure_driver_action(shell: &VegaShell, window: &gtk::ApplicationWindow, dbus: VegaDbus) {
...
fn configure_assistant(shell: &VegaShell, window: &gtk::ApplicationWindow, dbus: VegaDbus) {
...
async fn handle_assistant_mutation(
    ...
    let (title, description, confirm_label) = match call.name.as_str() {
        "install_package" => (
    ...
    let confirmed = match page.root.root().and_downcast::<gtk::ApplicationWindow>() {
        Some(window) => widgets::confirm(&window, &title, &description, &confirm_label).await,
        None => false,
    };
    if !confirmed {
        page.append(
...
#[cfg(test)]
mod tests {
    use super::{APPLICATION_ID, requires_pkgbuild_review};

    #[test]
    fn production_id_is_stable() {
        assert_eq!(APPLICATION_ID, "org.lyraos.VegaXfce");
    }
