...
export default function Network(): JSX.Element {
  ...
  async function applyStatic(): Promise<void> {
    const ok = await dialogs.confirm({
      ...
      message: `Aplicar ${staticForm.address} em ${staticForm.connection}?`,
      variant: 'warning',
        ...
        {interfaces.length === 0 ? (
          ...
          interfaces.map((iface) => (
            <div key={iface.device} style={{ display: 'grid', gridTemplateColumns: '1fr 1fr auto', gap: 12, alignItems: 'center', borderBottom: '1px solid var(--lyra-border)', paddingBottom: 10 }}>
              ...
              <button onClick={() => disconnect(iface.device)} disabled={busy} style={{ padding: '6px 12px', borderRadius: 'var(--lyra-radius-sm)', border: '1px solid var(--lyra-border)', background: 'transparent', color: 'var(--lyra-danger)' }}>Desconectar</button>
            </div>
          ))
        )}
      </div>
        ...
        {wifi.length === 0 ? (
          ...
          wifi.slice(0, 12).map((network) => (
            <div key={`${network.device}-${network.ssid}`} style={{ display: 'flex', gap: 12, alignItems: 'center' }}>
              ...
              <button onClick={() => connectWifi(network)} disabled={busy || network.active} style={{ padding: '6px 12px', borderRadius: 'var(--lyra-radius-sm)', border: '1px solid var(--lyra-border)', background: network.active ? 'transparent' : 'var(--lyra-gradient)', color: network.active ? 'var(--lyra-success)' : '#fff' }}>
                {network.active ? 'Conectada' : 'Conectar'}
              </button>
            </div>
          ))
        )}
      ...
      <div className="card" style={{ display: 'grid', gap: 10 }}>
        ...
        {services.map((service) => (
          <div key={service.name} style={{ display: 'flex', justifyContent: 'space-between', gap: 12 }}>
            ...
            <button onClick={() => toggleService(service)} disabled={busy} style={{ padding: '6px 14px', borderRadius: 'var(--lyra-radius-sm)', border: '1px solid var(--lyra-border)', background: service.enabled ? 'transparent' : 'var(--lyra-gradient)', color: service.enabled ? 'var(--lyra-danger)' : '#fff' }}>
